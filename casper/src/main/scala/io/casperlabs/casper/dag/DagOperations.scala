  ...
  def relation[F[_]: MonadThrowable](start: Message, target: Message)(
    ...
    if (start.messageHash == target.messageHash) Relation.equal.some.pure[F]
      ...
      (start.mainRank, target.mainRank) match {
        ...
        case (startRank, targetRank) if startRank < targetRank =>
          MAS.getAncestorAt(target, start.mainRank).map { blockAncestor =>
            if (blockAncestor.messageHash == target.messageHash) Relation.ancestor.some
            else none[Relation]
          }
        case (startRank, targetRank) if startRank > targetRank =>
          MAS.getAncestorAt(start, target.mainRank).map { targetAncestor =>
            if (targetAncestor.messageHash == start.messageHash) Relation.descendant.some
            else none[Relation]
          }
      }
    }
